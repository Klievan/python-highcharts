...
class Highmap(object):
    ...
    def add_data_set(self, data, series_type="map", name=None, is_coordinate=False, **kwargs):
        ...
        if is_coordinate:
            ...
            if self.map and not self.data_temp:
                series_data = Series([], series_type='map',
                                     **{'mapData': self.map})
                series_data.__options__().update(SeriesOptions(
                    series_type='map', **{'mapData': self.map}).__options__())
        ...
        series_data.__options__().update(SeriesOptions(
            series_type=series_type, **kwargs).__options__())
        self.data_temp.append(series_data)
    ...
    def add_data_from_jsonp(self, data_src, data_name='json_data', series_type="map", name=None, **kwargs):
        ...
        self.add_data_set(RawJavaScriptText(data_name),
                          series_type, name=name, **kwargs)
    ...
    def add_map_data(self, geojson, **kwargs):
        ...
        if self.data_is_coordinate:
            ...
            self.data_temp[0].__options__().update(
                {'mapData': MapObject(self.map)})
    ...
    def set_options(self, option_type, option_dict, force_options=False):
        ...
        if force_options:  # not to use unless it is really needed
            ...
            self.options.update(
                {'colorAxis': self.options.get('colorAxis', ColorAxisOptions())})
    ...
    def _get_jsmap_name(self, url):
        ...
        ret = urlopen(url)
        # return the name of map file, Ex. 'Highcharts.maps["xxx/xxx"]'
    ...
    def buildcontent(self):
        ...
        if self.drilldown_flag:
            self.drilldown_data = json.dumps(
                self.drilldown_data_temp, cls=HighchartsEncoder)
    ...
    def iframe(self):
        ...
        width = int(self.options['chart'].__dict__[
                    'width']) if self.options['chart'].__dict__.get('width') else 820
        height = int(self.options['chart'].__dict__[
                     'height']) if self.options['chart'].__dict__.get('height') else 520
