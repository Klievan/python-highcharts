...
class Highstock(object):
    ...
    def add_navi_series_from_jsonp(self, data_src=None, data_name='json_data', series_type="line", **kwargs):
        ...
        self.add_navi_series(RawJavaScriptText(
            self.jsonp_data), series_type, **kwargs)
    ...
    def buildcontent(self):
        ...
        self.data_list = [json.dumps(x, cls=HighchartsEncoder)
                          for x in self.data_temp]

        if self.navi_seri_flag:
            self.navi_seri = json.dumps(
                self.navi_seri_temp, cls=HighchartsEncoder)
